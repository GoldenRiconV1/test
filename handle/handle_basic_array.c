...
static char **process_entry(struct dirent *entry, file_data_t *file_data,
    flags_t *flag, has_file_flag_t *has_ff)
{
    struct stat file_stats;
    char *fp;

    fp = malloc(my_strlen(has_ff->path[0]) + my_strlen(entry->d_name) + 2);
    if (fp == NULL)
        return NULL;
    my_strcpy(fp, has_ff->path[0]);
    if (has_ff->path[0][my_strlen(has_ff->path[0]) - 1] != '/')
        my_strcat(fp, "/");
    ...
    get_file_info(entry, file_data, has_ff->path[0]);
    free(fp);
