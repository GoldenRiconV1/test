...
static char **process_entry(struct dirent *entry, file_data_t *file_data,
    flags_t *flag, const char *path)
{
    struct stat file_stats;
    char *fp;

    fp = malloc(my_strlen(path) + my_strlen(entry->d_name) + 2);
    if (fp == NULL)
        return NULL;
    my_strcpy(fp, path);
    ...
    int array_len, flags_t *flag, const char *path)
{
    ...
    for (int i = 0; entry != NULL && i < array_len; i++) {
        array[i] = process_entry(entry, file_data, flag, path);
        entry = readdir(dir_stream);
    }
...
{
    int array_len = dir_len(path, error_status);
    char ***array;

    if (array_len == -1)
        return NULL;
    array = dir_to_l_array(file_data, array_len, flag, path);
    if (array == NULL)
        return NULL;
