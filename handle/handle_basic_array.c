...
static char **process_entry(struct dirent *entry, file_data_t *file_data,
    flags_t *flag, has_file_flag_t *has_file_flag)
{
    struct stat file_stats;
    char *fp;

    fp = malloc(my_strlen(has_file_flag->path) + my_strlen(entry->d_name) + 2);
    if (fp == NULL)
        return NULL;
    my_strcpy(fp, has_file_flag->path);
    if (has_file_flag->path[my_strlen(has_file_flag->path) - 1] != '/')
        my_strcat(fp, "/");
    my_strcat(fp, entry->d_name);
    if (entry->d_name[0] != '.' || flag->a_flags == 1) {
        if (stat(fp, &file_stats) == 0)
            flag->blocks += file_stats.st_blocks;
    }
    get_file_info(entry, file_data, has_file_flag->path);
    free(fp);
    ...
    int array_len, flags_t *flag, has_file_flag_t *has_file_flag)
{
    ...
    for (int i = 0; entry != NULL && i < array_len; i++) {
        array[i] = process_entry(entry, file_data, flag, has_file_flag);
        entry = readdir(dir_stream);
    }
...
{
    int array_len = dir_len(has_file_flag);
    char ***array;
