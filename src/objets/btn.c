...
bool create_restart_btn(game_t *game)
{
    ...
    game->restart_btn_texture = sfTexture_createFromFile(path, NULL);
    if (!game->restart_btn_texture)
        return false;
    game->restart_btn_sprite = sfSprite_create();
    if (!game->restart_btn_sprite)
        return false;
    sfSprite_setTexture(game->restart_btn_sprite, game->restart_btn_texture,
        sfTrue);
    sfSprite_move(game->restart_btn_sprite, offset);
    return true;
}
