...
bool parser(data_t *data)
{
    ...
    data->my_uid = getuid();
    data->my_gid = getgid();
    if (data->flags == EMPTY || data->flags == U_FLAG || data->flags == U_AND_G_FLAG)
        get_uid_from_file(data);
    if (data->flags == EMPTY || data->flags == G_FLAG || data->flags == U_AND_G_FLAG)
        get_gid_from_file(data);
    if (!check_error(data))
        return false;
    if (data->flags != H_FLAG)
        print_test(data);
