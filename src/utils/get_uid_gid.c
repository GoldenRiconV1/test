...
static void fill_str_array(struct s *s, int i, int nbr_arg)
{
...
static bool file_parser(data_t *data, struct s *s, int nbr_arg)
{
...
static bool load_file_and_fill_buf(data_t *data, struct s *s, char *filepath)
{
...
static void find_group(data_t *data, struct s *s)
{
...
static void find_user(data_t *data, struct s *s)
{
    for (int i = 0; s->array[i] != NULL; i++) {
