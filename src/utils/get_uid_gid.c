...
{
    for (int i = 0; s->array[i] != NULL; i++) {
        if (atoi(s->array[i][0]) == data->my_uid) {
...
static void find_group_name(data_t *data, parser_t *s)
{
    for (int i = 0; s->array[i] != NULL; i++) {
        if (atoi(s->array[i][0]) == data->my_gid) {
            data->groupe_name = (s->array[i][0]);
            break;
        }
    }
}
