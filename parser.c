...
static void file_exists(const char *filepath)
{
    ...
    if (stat(filepath, &buffer) == 0) {
        ...
        // Si stat a échoué, on vérifie la valeur de errno
        if (errno == ENOENT) {
            printf("'%s' -> ❌ N'existe pas (Erreur: No such file or directory).\n", filepath);
        } else {
            // Gère d'autres erreurs possibles (ex: droits d'accès insuffisants, EACCES)
            printf("'%s' -> ❓ Erreur inattendue en vérifiant le fichier (code: %d).\n", filepath, errno);
        }
    }
}
